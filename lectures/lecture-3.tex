...
\begin{itemize}
	\item The {\textbi{quantum efficiency}} (QE) is the ratio of the actual
  ...
  500~nm.  
	\item The {\textbi{spectral response}} is the change in the output
  signal as a function of the wavelength of the input signal.
\item The {\textbi{charge transfer efficiency}} (\textbi{CTE}) specifies the 
...
\item The {\textbi{dark current}} represent the output from the
  non-illuminated detector. It is usually measured as a
  root-mean-square current.  
\item The {\textbi{dynamic range}} is the ratio of the saturation output
  to the dark current.  
\item The {\textbi{noise equivalent power}} (NEP) is the input radiative 
  flux that gives
...
\item The  {\textbi{detectivity}} (D) is the inverse NEP value, that is, the
  signal-to-noise ratio for unity intensity input radiation.  
\item  The {\textbi{normalized detectivity}} (D$^*$) is the detectivity
...
Many types of detectors base their properties on those of {\textbi{
...
The way a CCD works can be described in four steps.
\begin{enumerate}
...
\begin{itemize}
	\item As a {\textbi{photoconductor}} where a battery holds the external voltage
  to a constant value and the current is a linear function of the
  incident photon flux.
\item As a {\textbi{power-cell}} the diode is connected to a constant-load
  resistance, and the power output depends on the incident photon
  flux. This is the principle behind solar power cells.
\item In the {\textbi{photovoltaic}} mode current from from the diode
...
The analysis of the buried channel MOS capacitor with photoelectrons
...
columns of this array are collectively called \textbi{parallel registers}, where
one column is a parallel register. The row below the 3x3 array is called the
\textbi{serial register}, and is shielded from light.
...
electrons in the rightmost pixel to a \textbi{output amplifier}, e.g. a MOSFET
amplifier, that
...
The voltage is
converted to a binary number through the \textbi{analog-to-digital converter}
(\textbi{ADC}). Finally, the binary number is stored in some form of computer
...
Noise sources can be separated into four classes: {\textbi{intrinsic noise}} 
originating from the detector, {\textbi{signal noise}} arising from the character
of the of the incoming signal, {\textbi{external noise}} e.g. spurious signals
from cosmic rays and the like, {\textbi{processing noise}} from amplifiers and
