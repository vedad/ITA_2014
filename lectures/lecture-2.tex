...
%In order to find something one needs a system of coordinates. % Don't like the sentence.
In order to identify an object's position, one needs a system of coordinates.
%For determining the positions of the stars and planets where the 
...
%Our first task is to decide on a coordinate system and the position of the following:
...
%	\captionsetup{width=0.9\textwidth}
	...
	\caption{}
%		\caption{Three great circles are drawn: The equator (latitudinal
%			circle), and two
%			longitudinal circles that pass through a point on the zenith, $C'$.
%			At the intersections, $A',B',C'$ they form spherical angles which includes a spherical
%			triangle.}
...
%	\captionsetup{width=0.9\textwidth}
	...
	\caption{}
%		\caption{The figure illustrates different kinds of circles on a sphere.}
...
the observer or 
\[ PZ=90^\circ-\phi \]
where $\phi$ is the observer's latitude. Hence the altitude of the pole
...
When we know the declination of a star, a small circle, {\bf the
...
\begin{comment}
In figure~\ref{fig:cel-sph-ecl}, the ecliptic and its inclination to the celestial
equator, the \textbf{obliquity of the ecliptic}, is drawn. Relative to the earth,
the sun appears to move on the celestial sphere along the ecliptic in
the direction as shown, and twice yearly its position coincides
...
to north, is the {\bf vernal equinox}. It is in this way that the
...
declination are both zero, which occurs roughly March 20.
At the highest 
right ascension is $6^{\mathrm{h}}$ and declination about $23.5^\circ$~N
(roughly June 21, summer solstice); at autumnal equinox (also called the first point of
Libra) the right ascension is
$12^{\mathrm{h}}$ and declination $0^\circ$ (September 21, autumnal equinox),
and at $Y$ the right ascension is $18^h$ and the declination about $23.5^\circ$~S (December 21, winter solstice).
\end{comment}
...
When the sun is on the meridian of a given place, it is \textbf{apparent
...
The {\bf tropical year} is the average interval between two consecutive 
