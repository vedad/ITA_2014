...
differentiate between two types of coordinate sytems: The \textbi{horizontal coordinate system} and
the \textbi{equatorial coordinate system}. A big difference between these two types
...
circle which is called a \textbi{great circle}. Any other plane that cuts
the sphere, but that does not pass through the center is a {\textbi{small
circle}}. When two great circles intersect at a point they are said
to include a {\textbi{spherical angle}} which is defined between the
...
%	\captionsetup{width=0.9\textwidth} Shorter caption width.
	\centering
	\includestandalone[scale=0.8]{celestial-sphere}
	\caption{}
...
%	\captionsetup{width=0.9\textwidth}
	\centering
	\includestandalone[scale=0.8]{celestial-sphere-circles}
	\caption{}
...
{\textbi{spherical triangle}}, as shown in Figure~\ref{fig:cel-sph-tri}.
...
We define the \textbi{zenith}, $Z$, to be the direction
directly overhead, while the opposite direction is the \textbi{nadir}. The fundamental plane
...
\begin{figure}[htpb]
	\centering
	\includestandalone[scale=1.0]{celestial-sphere-test}
...
the celestial sphere in the \textbi{north celestial pole} and \textbi{south celestial pole}.
...
$ZX=90^\circ-a$. This angle, $ZX$, is called the {\textbi{zenith distance}} of the star
$X$. Drawing $LXM$ as a small circle through $X$ parallel to the
horizon, we get what is called the {\textbi{parallel of altitude}}. 
...
point $N$ as the {\textbi{north point of the horizon}}.
...
$180^\circ$ is called the {\textbi{spherical excess}}.

Consider a spherical triangle, such as Figure~\ref{fig:spherical-triangle}, with corners $A,B,C$ on a unit
...
$W$ and $E$ are the remaining {\textbi{cardinal points}}, in addition to
$N$ and $S$. 
...
sphere. Then $DX$ is called the {\textbi{declination}} of the star and is
{\textbi{north declination}} if the star is between the celestial equator and
the north pole. Denoting the stars declination $DX$ by $\delta$ and
$PX=90^\circ-\delta$ is called the {\textbi{north polar distance}} of the
star. 

When we know the declination of a star, a small circle, {\textbi{the
...
{\textbi{observer's meridian}}. When the star is at $L$ on the observers
meridian it is is said to {\textbi{transit}} or {\textbi{culminate}} at which
...
{\textbi{set}} with an altitude of $0^\circ$. Eventually it will pass through
its position of maximum depression at $M$ before continuing on to $G$
where the altitude again is $0^\circ$ and is said to {\textbi{rise}}. At
...
{\textbi{hour angle}} and is measured from the observer's meridian
...
{\textbi{vernal equinox}} or the {\textbi{first point of Aries}}. The arc
$\aries D$ is called the {\textbi{right ascension}} ({\sc r.a.}) of the
...
the {\textbi{sidereal time}}\footnote{{\textbi{Sidereal}}: of or with respect to distant stars.} 
...
of the stars. The plane of the orbit is called the {\textbi{plane of the
...
{\textbi{ecliptic}}. With reference to the stars, the plane of the ecliptic
...
\begin{figure}[htpb]
	\centering
	\includestandalone[scale=1.0]{celestial-sphere-ecliptic}
...
equator, the \textbi{obliquity of the ecliptic}, is drawn. Relative to the earth, the sun
...
south to north (negative to positive), is the \textbi{vernal equinox}. It is in this way
...
reach June 20th, which is the \textbi{summer solstice}; the longest day of the year
...
celestial equator is the \textbi{first point of Libra}, which is more commonly known as the
\textbi{autumnal equinox}, and occurs roughly September 23. At this point, the declination
and right ascension of the sun are zero again. December 21 marks the shortest day of the
year in the northern hemisphere, and is called the \textbi{winter solstice}. At this
...
sun's annual motion (eastwards), is called the {\textbi{longitude}} of $X$
and is measured from $0^\circ$ to $360^\circ$ round the ecliptic. The
arc $\Lambda X$ is the {\textbi{latitude}}, north is considered positive
and south negative. 
%
\begin{figure}[h]
	\centering
...
When the sun is on the meridian of a given place, it is \textbi{apparent
noon} there; when the sun is next on the meridian, an {\textbi{apparent
...
year is called a {\textbi{mean solar day}}. The mean sun is assumed to
...
When the mean sun is on the meridian of Greenwich, it is {\textbi{Greenwich
mean noon}}. The hour angle of the mean sun at Greenwich is denoted
	{\sc g.m.a.t.} (\textbi{Greenwich mean astronomical time}). 
Mean time reckoned from midnight at Greenwich is called
{\textbi{Greenwich Mean Time}} ({\sc g.m.t.}), now designated {\textbi{
...
Greenwich. The sidereal time at $l$ is called the {\textbi{local sidereal
time}} ({\sc l.s.t.}).
...
equinox as a fixed point. However, there are both the phenomena of {\textbi{
precession}} and {\textbi{nutation}}. Precession is the slow movement of the axis of a spinning body around another axis due to a torque acting to change the direction of the first axis.
...
are called the {\textbi{mean equinoxes}} at the dates
...
the interval given by a fixed equinox. This first interval is a {\textbi{sidereal day}}, 
...
One defines {\textbi{mean sidereal time}} to associated with the moving mean equinox 
(only precession considered) and {\textbi{apparent sidereal time}} to be associated 
...
rotation of the earth and {\textbi{ephemeris time}} which is uniform and is defined
...
account, the {\textbi{fictitious mean sun}} is defined to travel along the mean 
...
ecliptic is called a {\textbi{sidereal year}}. 

The {\textbi{tropical year}} is the average interval between two consecutive 
...
the civil year and is usually called the {\textbi{Besselian year}}. 
...
number of days which have elapsed since this epoch defines the {\textbi{Julian Date}}
...
through the observation of {\textbi{stellar parallax}}. As the Earth travels in its orbit, the apparent
...
\begin{figure}[htpb]
	\centering
%	\includestandalone[scale=1.0]{parallax-illustration}
...
{\bf Some history:} James Bradley FRS (March 1693 -– 13 July 1762) was an English astronomer the Astronomer Royal from 1742. He undertook to measure stellar parallax. Bradley could measure stellar positions with a precision of about 0.5~arcsec (500~mas). This was good enough to
